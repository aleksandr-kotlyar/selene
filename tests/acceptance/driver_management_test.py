...
@pytest.fixture(scope='function')
def a_browser_built_with__rebuild_not_alive_driver__enabled():
    yield selene.Browser(selene.Config(rebuild_not_alive_driver=True))


@pytest.fixture(scope='function')
def a_browser_built_with__reset_not_alive_driver_on_get_url__disabled():
    yield selene.Browser(selene.Config(_reset_not_alive_driver_on_get_url=False))
...
def test_driver_built_on_browser__will_be_killed_even__if_copied_with_hold_driver_if(
    ...
    driver = persistent.Field.value_from(browser.config, 'driver')
    assert driver is persistent.Field.value_from(copied.config, 'driver')
    pytest.raises(MaxRetryError, lambda: driver.title)  # KILLED
...
def test_first_access_to_driver_via_browser_quit__builds_and_kills_driver(
    ...
    browser = selene.Browser(
        selene.Config(
            # rebuild_not_alive_driver=False,  # <- GIVEN kept as default
        )
    )
    ...
    #     GIVEN previously built browser with Config(rebuild_not_alive_driver=False)
    pytest.raises(MaxRetryError, lambda: browser.driver.title)


def test_can_rebuild_browser_on_next_open_after_its_death(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config())
    browser.quit()
    dead_driver = persistent.Field.value_from(browser.config, 'driver')

    browser.open(empty_page)
    driver = persistent.Field.value_from(browser.config, 'driver')

    assert driver is not dead_driver
    assert driver.title == 'Selene Test Page'  # alive
    assert driver is browser.driver


def test_can_not_rebuild_browser_on_next_open_after_its_death_if(
    a_browser_built_with__reset_not_alive_driver_on_get_url__disabled,
    with_process_exit_teardown,
):
    browser = a_browser_built_with__reset_not_alive_driver_on_get_url__disabled
    browser.quit()
    dead_driver = persistent.Field.value_from(browser.config, 'driver')

    pytest.raises(MaxRetryError, lambda: browser.open(empty_page))
    driver = persistent.Field.value_from(browser.config, 'driver')

    assert driver is dead_driver
    pytest.raises(MaxRetryError, lambda: driver.title)  # dead


def test_can_not_rebuild_browser_on_first_access_after_its_death(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config())
    browser.quit()
...
def test_can_rebuild_browser_on_first_access_after_its_death_if(
    a_browser_built_with__rebuild_not_alive_driver__enabled,
    with_process_exit_teardown,
):
    browser = a_browser_built_with__rebuild_not_alive_driver__enabled
    browser.quit()
...
def test_browser_remains_dead_if_configured_with_not_rebuild_not_alive_driver_on_init(
    with_process_exit_teardown,
):
    browser = selene.Browser(
        selene.Config(
...
def test_browser_remains_dead_if_post_configured_for_not_rebuild_not_alive_driver(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config(rebuild_not_alive_driver=True))  # <- GIVEN
    browser.quit()

    browser.config.rebuild_not_alive_driver = False  # <- WHEN
    driver = browser.driver
...
def test_browser_copy_remains_dead__if_copied_with_not_rebuild_not_alive_driver_if(
    a_browser_built_with__override_driver_with_all_driver_like_options__disabled,
    with_process_exit_teardown,
):
    browser = (
    ...
    browser.config.rebuild_not_alive_driver = True  # <- GIVEN
    browser.quit()

    copy = browser.with_(rebuild_not_alive_driver=False)  # <- WHEN
...
def test_browser_resurrects_itself_and_copy__after_copy_programmed_to_death_forever_if(
    ...
    """
    AND if call browser.open() not just browser.driver
    """
    browser = (
        a_browser_built_with__override_driver_with_all_driver_like_options__disabled
    )
...
def test_browser_driver_access_rebuilds_or_not_depending_on_rebuild_not_alive_driver(
    ...
    # direct browser.driver access should rebuild after enabling the option
...
def test_can_build_second_driver_if_previous_was_forgotten_via_reset_to_none(
    with_process_exit_teardown,
):
    # GIVEN
    browser = selene.Browser(selene.Config())
...
def test_driver_can_be_manually_set_on_init_and_live_to_death_on_quit(
    ...
    assert manual_driver is persistent.Field.value_from(browser.config, 'driver')
...
def test_should_kill_at_exit_all_drivers_including_manually_set_and_forgotten(
    with_process_exit_teardown, manual_driver
):
    browser = selene.Browser(selene.Config())
    first_driver = browser.driver
    browser.config.driver = manual_driver
    browser.config.driver = manual_driver  # just in case:) TODO: move to separate test
    browser.config.driver = ...
...
def test_build_post_tuned_driver_by_name_on_next_open_after_first_quit(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config())
    driver = browser.driver
...
def test_build_post_tuned_driver_by_name_on_second_access_after_first_quit_if(
    a_browser_built_with__rebuild_not_alive_driver__enabled,
    with_process_exit_teardown,
):
    browser = a_browser_built_with__rebuild_not_alive_driver__enabled
...
def test_build_post_tuned_driver_by_name_on_2nd_access_after_1st_reset_with_ellipsis(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config())
    driver = browser.driver
...
def test_build_post_tuned_driver_by_name_on_2nd_access_after_1st_reset_with_none(
    with_process_exit_teardown,
):
    browser = selene.Browser(selene.Config())
    driver = browser.driver
...
def test_firefox_built_via_copy_is_kept_for_original_browser__if_preconfigured(
    ...
    browser = selene.Browser(
        selene.Config(_override_driver_with_all_driver_like_options=False)  # <- GIVEN
    )
...
def test_post_configured_sharing_driver_between_original_and_copy_when_only_name_differs(
    ...
    '''
    origin.config._override_driver_with_all_driver_like_options = False  # <- GIVEN
...
def test_preconfigured_new_driver_for_copy_by_explicit_driver_set_on_copying_nothing(
    with_process_exit_teardown,
):
    origin = selene.Browser(
        selene.Config(
...
def test_new_driver_for_copy_by_explicit_driver_set_ellipsis_on_copying_nothing(
    with_process_exit_teardown,
):
...
def test_new_driver_for_copy_by_explicit_driver_set_none_on_copying_nothing(
    with_process_exit_teardown,
):
    origin = selene.Browser(selene.Config())  # <- GIVEN
    copied = origin.with_(driver=None)  # <- AND
...
def test_post_configured_new_driver_to_copy_by_explicit_driver_set_on_copying_with_name(
    with_process_exit_teardown,
):
    origin = selene.Browser(selene.Config())
    origin.config._override_driver_with_all_driver_like_options = False  # <- GIVEN
...
def test_new_driver_to_copy_by_implicit_driver_set_on_cloning_with_name(
    with_process_exit_teardown,
):
    origin = selene.Browser(selene.Config())
    '''
